...
class S4Manager:
    ...
    def __init__(self, config_path="config/account.yaml", hostname=None):
        ...
        self.wait_time = self.config.get(
            "s4_wait_time", 10
        )  # 系统唤醒后等待恢复的时间（秒），确保SSH服务稳定、系统服务就绪
        self.s4_timeout = self.config.get(
            "s4_timeout", 600
        )  # S4唤醒超时时间（秒），默认10分钟（S4比S3慢）
        self.s4_wake_delay = self.config.get(
            "s4_wake_delay", 60
        )  # S4自动唤醒延迟时间（秒），默认60秒
    ...
    def _wait_for_ssh(self, max_wait_time=None):
        ...
        Args:
            max_wait_time: 最大等待时间（秒），默认使用配置中的s4_timeout

        Returns:
            bool: SSH服务是否可用
        """
        if max_wait_time is None:
            max_wait_time = self.s4_timeout
        ...
        try:
            ...
            try:
                stdin, stdout, stderr = self.ssh.exec_command(
                    "sudo -n systemctl hibernate 2>&1", timeout=2
                )
    ...
    def _execute_s4_command(self):
        """执行S4命令并确认执行状态"""
        self.logger.info(f"{self.hostname}\t正在执行S4（休眠到磁盘）命令...")

        # 检查是否需要sudo
        need_sudo = self._check_user_permissions()

        # 尝试多种S4命令方式
        # 优先级：rtcwake（支持定时唤醒） > systemctl hibernate > echo disk
        s4_commands = []
        if need_sudo:
            s4_commands = [
                f"sudo -S rtcwake -m disk -s {self.s4_wake_delay}",  # RTC定时唤醒，指定时间后自动唤醒
                "sudo -S systemctl hibernate",
                "sudo -S sh -c 'echo disk > /sys/power/state'",
            ]
        else:
            s4_commands = [
                f"rtcwake -m disk -s {self.s4_wake_delay}",  # RTC定时唤醒，指定时间后自动唤醒
                "systemctl hibernate",
                "sh -c 'echo disk > /sys/power/state'",
            ]

        last_error = None
        for command in s4_commands:
            try:
                if need_sudo and "-S" in command:
                    self.logger.info(
                        f"{self.hostname}\t尝试使用sudo执行S4命令: {command.split()[1]}"
                    )
                else:
                    self.logger.info(f"{self.hostname}\t尝试执行S4命令: {command}")
                ...
                # 等待命令开始执行（S4命令执行后连接会断开）
                exit_status = None
                stdout_output = ""
                stderr_output = ""

                try:
                    # 尝试读取输出（可能为空，因为S4会立即休眠）
                    stderr_output = (
                        stderr.read().decode("utf-8", errors="ignore").strip()
                    )

                    # 等待命令状态（S4通常不会返回，连接会断开）
                    try:
                        exit_status = stdout.channel.recv_exit_status()
                    except Exception:
                        # 对于S4命令，连接断开是正常的，exit_status可能无法获取
                        pass
                ...
                if exit_status is not None:
                    if exit_status == 0:
                        self.logger.info(
                            f"{self.hostname}\tS4命令执行成功（退出码: {exit_status}）"
                        )
                        return True
                    else:
                        self.logger.warning(
                            f"{self.hostname}\tS4命令执行失败（退出码: {exit_status}），尝试下一个命令"
                        )
                    ...
                    if stderr_output and (
                        ...
                        self.logger.warning(
                            f"{self.hostname}\tS4命令可能执行失败: {stderr_output}，尝试下一个命令"
                        )
                        ...
                        # 连接断开通常是S4成功的标志
                        self.logger.info(
                            f"{self.hostname}\tS4命令已发送（连接已断开，这是S4命令的正常行为）"
                        )
                        return True

            except Exception as e:
                self.logger.debug(
                    f"{self.hostname}\t执行S4命令 '{command}' 时出错: {str(e)}，尝试下一个命令"
                )
                last_error = str(e)
                continue

        # 所有命令都失败了
        raise Exception(f"所有S4命令都执行失败，最后一个错误: {last_error}")

    def _verify_s4_entered(self, s4_start_time=None):
        """
        验证S4是否真正生效（系统是否真的进入了S4状态）

        注意：只检查S4开始执行之后的新日志记录，避免受到之前S4操作的影响

        Args:
            s4_start_time: S4开始执行的时间戳（datetime对象或时间戳字符串），用于只检查该时间之后的日志

        Returns:
            tuple: (bool, str) - (是否进入S4, 验证信息)
        """
        try:
            self.logger.info(f"{self.hostname}\t正在验证S4是否真正生效...")
            self.ssh = connect(self.host, self.user, self.password)

            verification_results = []

            # 计算时间范围：如果提供了S4开始时间，只检查该时间之后的日志
            # 否则检查最近3秒内的日志（避免检查到之前S4的记录）
            if s4_start_time:
                # 将时间戳转换为适合journalctl的格式
                if isinstance(s4_start_time, datetime):
                    time_str = s4_start_time.strftime("%Y-%m-%d %H:%M:%S")
                else:
                    time_str = str(s4_start_time)
                time_filter = f"--since '{time_str}'"
                time_desc = f"S4开始时间({time_str})之后"
            else:
                # 默认只检查最近3秒内的日志（避免检查到之前S4的记录）
                time_filter = "--since '3 seconds ago'"
                time_desc = "最近3秒内"

            # 方法1: 检查dmesg日志中是否有hibernate/resume记录
            # 注意：为了避免检查到之前S4的记录，使用dmesg -T获取时间戳并进行过滤
            try:
                # 使用dmesg -T获取带时间戳的输出
                # 先获取更多记录，然后根据时间戳过滤
                dmesg_cmd = "dmesg -T 2>/dev/null | grep -iE '(hibernate|resume|s4|s4_state)' | tail -10"
                dmesg_result = command_execute(self.ssh, dmesg_cmd).strip()
                if dmesg_result:
                    # 如果提供了S4开始时间，解析dmesg的时间戳进行精确过滤
                    if s4_start_time and isinstance(s4_start_time, datetime):
                        # 解析dmesg -T输出格式: [Mon Jan 01 12:00:00 2024] ...
                        # 提取时间戳并与S4开始时间比较，只保留S4开始时间之后的记录
                            ...
                            try:
                                ...
                                if timestamp_match:
                                    ...
                                    try:
                                        ...
                                        # 只保留S4开始时间之后的记录（加上2秒容差，因为命令执行需要时间）
                                        from datetime import timedelta

                                        if dmesg_time >= (
                                            s4_start_time - timedelta(seconds=2)
                                        ):
                        ...
                        if recent_lines:
                            recent_result = "\n".join(recent_lines)
                            if (
                                "hibernate" in recent_result.lower()
                                or "s4" in recent_result.lower()
                            ):
                                verification_results.append(
                                    "✓ dmesg显示有hibernate记录（本次S4）"
                                )
                                self.logger.info(
                                    f"{self.hostname}\t✓ dmesg验证通过: 发现本次S4的hibernate记录"
                                )
                            else:
                                verification_results.append(
                                    "⚠ dmesg未发现本次S4的hibernate记录"
                                )
                        else:
                            verification_results.append(
                                "⚠ dmesg无本次S4时间范围内的hibernate记录"
                            )
                    else:
                        ...
                        last_result = "\n".join(last_lines)
                        if (
                            "hibernate" in last_result.lower()
                            or "s4" in last_result.lower()
                        ):
                            verification_results.append(
                                "✓ dmesg显示有hibernate记录（最新记录）"
                            )
                            self.logger.info(
                                f"{self.hostname}\t✓ dmesg验证通过: 发现最新的hibernate记录"
                            )
                        else:
                            verification_results.append(
                                "⚠ dmesg未发现最新的hibernate记录"
                            )
                            self.logger.warning(
                                f"{self.hostname}\t⚠ dmesg验证: 未发现最新的hibernate记录"
                            )
                else:
                    verification_results.append("⚠ dmesg无hibernate相关记录")
                    self.logger.warning(
                        f"{self.hostname}\t⚠ dmesg验证: 无hibernate相关记录"
                    )
            except Exception as e:
            ...
            # 使用时间过滤，只检查S4开始之后的日志
            try:
                journal_cmd = f"journalctl {time_filter} | grep -iE '(hibernate|resume|s4)' | tail -3"
                journal_result = command_execute(self.ssh, journal_cmd).strip()
                if journal_result:
                    if (
                        "hibernate" in journal_result.lower()
                        or "s4" in journal_result.lower()
                    ):
                        verification_results.append(
                            f"✓ journalctl显示有hibernate记录（{time_desc}）"
                        )
                        self.logger.info(
                            f"{self.hostname}\t✓ journalctl验证通过: 发现{time_desc}的hibernate记录"
                        )
                    else:
                        verification_results.append(
                            f"⚠ journalctl未发现{time_desc}的hibernate记录"
                        )
                else:
                    verification_results.append(
                        f"⚠ journalctl无{time_desc}的hibernate相关记录"
                    )
            except Exception as e:
                self.logger.debug(f"{self.hostname}\t检查journalctl时出错: {str(e)}")

            # 方法3: 检查/sys/power目录状态
            try:
                # 检查是否有power相关的设备信息
                power_state_cmd = "cat /sys/power/state 2>/dev/null || echo 'N/A'"
                power_state = command_execute(self.ssh, power_state_cmd).strip()
                if "disk" in power_state.lower():
                    verification_results.append("✓ /sys/power/state包含disk（S4支持）")
                    self.logger.info(
            ...
            if success_count > 0:
                ...
                self.logger.info(
                    f"{self.hostname}\tS4验证通过 ({success_count}/{len(verification_results)}项通过): {verification_msg}"
                )
                ...
                self.logger.warning(
                    f"{self.hostname}\tS4验证未通过: {verification_msg}"
                )
                return False, verification_msg

        except Exception as e:
            self.logger.warning(f"{self.hostname}\t验证S4时出错: {str(e)}")
            self._safe_disconnect()
            return False, f"验证过程出错: {str(e)}"

    def _wait_for_s4_completion(self, s4_start_time=None):
        """
        等待S4完成（系统唤醒）并验证S4是否生效

        Args:
            s4_start_time: S4开始执行的时间戳（用于验证时只检查S4之后的新日志）
        """
        # 等待系统唤醒（检测SSH服务是否可用）
        ssh_ready = self._wait_for_ssh()

        if ssh_ready:
            # 系统唤醒后，等待一段时间让系统完全恢复（SSH服务稳定、系统服务就绪）
            self.logger.info(
                f"{self.hostname}\t系统已唤醒，等待 {self.wait_time} 秒让系统完全恢复..."
            )
            time.sleep(self.wait_time)

            # 验证S4是否真正生效
            # 只检查S4开始之后的新日志记录
            s4_verified, verification_msg = self._verify_s4_entered(s4_start_time)
            if not s4_verified:
                self.logger.warning(
                    f"{self.hostname}\t警告: 系统已唤醒，但未找到S4生效的证据。"
                    f"这可能表示S4命令未真正执行，或者系统日志不完整。"
    ...
    def s4(self, hostname=None, wait_for_completion=True):
        """
        执行S4（休眠到磁盘）
        ...
        try:
            ...
            # 记录S4开始时间，用于后续验证时只检查本次S4的日志（避免受到之前S4记录的影响）
            s4_start_time = datetime.now()

            # 执行S4命令（该方法会检查命令执行状态）
            try:
                self._execute_s4_command()
            except Exception as e:
                self.logger.error(f"{self.hostname}\tS4命令执行失败: {str(e)}")
                raise

            self._safe_disconnect()

            if wait_for_completion:
                return self._wait_for_s4_completion(s4_start_time)
            self.logger.info(f"{self.hostname}\tS4命令已发送，不等待完成")
            return True
    ...
    def _handle_ssh_exception(self, e):
        ...
        if isinstance(e, paramiko.AuthenticationException):
            ...
            self.logger.error(
                f"{self.hostname}\tS4过程中发生错误: {str(e)}", exc_info=True
            )
...
def s4_system(hostname, config_path="config/account.yaml", wait_for_completion=True):
    """
    便捷函数：对指定主机执行S4
    ...
    使用示例:
        from utils.s4 import s4_system

        # 执行S4并等待系统唤醒
        success = s4_system("z")

        # 只发送S4命令，不等待
        success = s4_system("y", wait_for_completion=False)
    """
    manager = S4Manager(config_path, hostname=hostname)
    return manager.s4(wait_for_completion=wait_for_completion)


def s4_all_hosts(config_path="config/account.yaml", wait_for_completion=True):
    """
    便捷函数：对所有主机执行S4
    ...
    Returns:
        dict: 每个主机的S4结果 {hostname: success}

    使用示例:
        from utils.s4 import s4_all_hosts

        # 对所有主机执行S4并等待唤醒
        results = s4_all_hosts()
    """
    manager = S4Manager(config_path)
    results = {}
    ...
    for hostname in manager.hosts.keys():
        try:
            ...
            manager.logger.error(f"{hostname}\tS4失败: {str(e)}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 执行S4
        logger.info("\n2. 执行S4操作:")
        success = manager.s4(hostname=test_hostname, wait_for_completion=True)
