"""
500次循环S3（挂起到内存）测试脚本 - 支持多台机器并行测试

测试用例：麒麟系统P10 S3（挂起到内存）500次循环测试

测试规则：
- 自动要求2台机器运行500次循环（可配置）
- 允许有1次非Critical且S3可恢复的Fail（包括蓝屏、黑屏、系统Hang住等）
- 不允许有Critical及S3无法恢复的Fail

测试步骤（实现方式）：
1. 初始化阶段：
   - 加载配置文件（config/account.yaml），读取主机信息、用户凭证、循环次数等
   - 初始化S3Manager管理器，建立SSH连接准备
...
2. 循环测试阶段（500次）：
   ...
   b) 执行S3命令：
      - 检测用户权限（是否需要sudo）
      - 尝试多种S3命令方式（按优先级顺序）：
        * systemctl suspend（systemd方式）
        * rtcwake -m mem -s 30（RTC定时唤醒，30秒后自动唤醒）
      - S3命令执行后SSH连接会断开（这是正常行为）

   c) 等待系统唤醒：
      - 等待系统完全进入S3状态（wait_time秒）
      - 持续检测SSH服务是否可用（最多等待s3_timeout秒）
      - 每10秒记录一次等待日志
      - 系统唤醒后重新建立SSH连接

   d) 验证S3是否生效：
      - 系统唤醒后，通过多种方式验证S3是否真正生效：
        * 检查dmesg日志中的suspend/resume记录（最近1分钟内的日志）
        * 检查journalctl日志（systemd系统）中的suspend记录
        * 检查/sys/power/state文件确认S3支持（mem状态）
      - 如果未找到S3生效的证据，记录警告（可能是日志不完整或S3未真正执行）
   ...
   f) 错误处理：
      ...
      - 对于可恢复错误，等待30秒后尝试重新连接验证，如果失败则尝试强制S3
...
5. 报告生成：
   - 自动生成HTML报告（包含汇总统计、每台主机详细统计、每次S3耗时详情）
   - 自动生成Excel报告（包含汇总统计表和每台主机的详细数据）
   - 报告文件保存在reports/目录下

预期结果：
1. 测试通过标准：
   - 所有500次S3操作均成功完成，系统能够正常唤醒
   - 或者最多允许1次非Critical且可恢复的失败（如临时网络问题）
   - 不允许有任何Critical失败
   - 不允许有任何不可恢复的失败（如系统蓝屏、死机等）

2. 系统行为：
   - 每次S3后系统能够正常唤醒（在s3_timeout时间内）
   - SSH服务能够正常恢复
...
4. 报告输出：
   - 控制台输出详细的测试日志和最终统计报告
   - 生成HTML格式的测试报告（包含所有统计信息和每次S3的详情）
...
class S3CycleTest:
    ...
    def __init__(
        ...
        """
        self.manager = S3Manager(config_path)
        ...
        for hostname in self.hostnames:
            self.stats[hostname] = {
                ...
                "end_time": None,
                "failures": [],  # 记录失败的信息
                "cycle_durations": [],  # 记录每次S3的耗时（成功和失败都记录）
            }
    ...
    def _check_recoverable(self, hostname, failure_info):
        ...
        # 如果无法恢复，尝试强制S3
        self.logger.warning(f"{hostname}\t常规恢复失败，尝试强制S3...")
        try:
            success = self.manager.s3(hostname=hostname, wait_for_completion=True)
            if success:
                self.logger.info(f"{hostname}\t✓ 强制S3成功，故障已恢复")
                return True
        except Exception as e:
            self.logger.error(f"{hostname}\t✗ 强制S3也失败: {str(e)}")
    ...
    def _run_single_host_test(self, hostname):
        ...
        for cycle in range(1, self.total_cycles + 1):
            ...
            self.logger.info(f"\n{hostname}\t第 {cycle}/{self.total_cycles} 次S3测试")
            ...
            try:
                ...
                if success:
                    ...
                    self.logger.info(
                        f"{hostname}\t✓ 第 {cycle} 次S3成功，耗时 {cycle_duration:.2f} 秒"
                    )

                    # 验证S3后系统状态
                    try:
                        status = self.manager.verify_system_status(hostname=hostname)
                        self.logger.debug(f"{hostname}\tS3后系统状态: {status}")
                    except Exception as e:
                        self.logger.warning(
                            f"{hostname}\t第 {cycle} 次S3后验证系统状态失败: {str(e)}"
                        )

                else:
                    # S3失败
                    cycle_duration = time.time() - cycle_start_time
                    error_msg = "S3失败（返回False）"
                    ...
                    if failure_type == FailureType.NON_CRITICAL_RECOVERABLE:
                        self.logger.info(
                            f"{hostname}\t第 {cycle} 次S3失败，尝试恢复检测..."
                        )
                ...
                if failure_type == FailureType.NON_CRITICAL_RECOVERABLE:
                    self.logger.info(
                        f"{hostname}\t第 {cycle} 次S3异常，尝试恢复检测..."
                    )
                    is_recoverable = self._check_recoverable(hostname, None)
                    # 如果恢复检测失败，更新为不可恢复
                    if not is_recoverable:
    ...
    def _record_failure(self, hostname, failure_info):
        ...
        if failure_info.failure_type == FailureType.CRITICAL:
            ...
            self.logger.error(
                f"{hostname}\t✗ 第 {failure_info.cycle} 次S3失败 [CRITICAL]: {failure_info.error}"
            )
        elif failure_info.recoverable:
            self.stats[hostname]["non_critical_recoverable_failures"] += 1
            self.logger.warning(
                f"{hostname}\t⚠ 第 {failure_info.cycle} 次S3失败 [Non-Critical, Recoverable]: {failure_info.error}"
            )
        else:
            self.stats[hostname]["non_critical_unrecoverable_failures"] += 1
            self.logger.error(
                f"{hostname}\t✗ 第 {failure_info.cycle} 次S3失败 [Non-Critical, Unrecoverable]: {failure_info.error}"
            )
    ...
    def run_test(self):
        """运行单台或多台机器的循环S3测试（多台时并行）"""
        self.logger.info("=" * 80)
        if len(self.hostnames) == 1:
            self.logger.info(f"开始500次循环S3测试（单台机器）")
        else:
            self.logger.info(f"开始500次循环S3测试（{len(self.hostnames)}台机器并行）")
    ...
    def _print_final_report(self, start_timestamp):
        ...
        self.logger.info("\n" + "=" * 80)
        self.logger.info("500次循环S3测试 - 最终报告")
        self.logger.info("=" * 80)
        ...
        for hostname in self.hostnames:
            ...
            if stats["failures"]:
                ...
                for failure in stats["failures"]:
                    ...
                    self.logger.error(
                        f"    第 {failure.cycle} 次S3失败 [{failure_type_desc}]: {failure.error}"
                    )
                    self.logger.error(
        ...
        # 判断测试结果
        # 规则：允许有1次非Critical且S3可恢复的Fail，不允许有Critical及S3无法恢复的Fail
        if total_critical > 0:
            self.logger.error(
        ...
        self.logger.info("✓ 测试通过：所有S3均成功，无任何失败")
        return True
...
if pytest is not None:
    ...
    def test_s3_cycles(config_path, required_hosts):
        """
        循环S3测试用例
